...
# Version: 2.1
# Modified: 2025-12-15
# Changes: Added local directory binary detection for dialog and utiluti
#
...
# 2.1 - Enhanced binary detection: checks local directory first, then /usr/local/bin
...
# Binary Detection - Check local directory first, then /usr/local/bin
# This allows for portable installations or testing without system-wide installation

# Detect utiluti binary
if [[ -x "${SCRIPT_DIR}/utiluti" ]]; then
    UTI_COMMAND="${SCRIPT_DIR}/utiluti"
    echo "Using local utiluti: ${UTI_COMMAND}"
elif [[ -x "/usr/local/bin/utiluti" ]]; then
    UTI_COMMAND="/usr/local/bin/utiluti"
    echo "Using system utiluti: ${UTI_COMMAND}"
else
    UTI_COMMAND="/usr/local/bin/utiluti"  # Default for error message
fi

# Detect dialog binary
if [[ -x "${SCRIPT_DIR}/dialog" ]]; then
    SW_DIALOG="${SCRIPT_DIR}/dialog"
    echo "Using local dialog: ${SW_DIALOG}"
elif [[ -x "/usr/local/bin/dialog" ]]; then
    SW_DIALOG="/usr/local/bin/dialog"
    echo "Using system dialog: ${SW_DIALOG}"
else
    SW_DIALOG="/usr/local/bin/dialog"  # Default for installation
fi
...
function check_swift_dialog_install ()
{
    ...
    # Checks local directory first, then /usr/local/bin
    #
    # RETURN: None

    logMe "Ensuring that swiftDialog version is installed..."
    logMe "Using swiftDialog at: ${SW_DIALOG}"
...
function check_utiluti_install ()
{
    ...
    logMe "Checking for utiluti at: ${UTI_COMMAND}"

    if [[ ! -x "${UTI_COMMAND}" ]]; then
        logMe "ERROR: utiluti is not installed at ${UTI_COMMAND}"
        echo ""
        echo "========================================="
        echo "ERROR: utiluti not found"
        echo "========================================="
        echo ""
        echo "utiluti was not found in:"
        echo "  - ${SCRIPT_DIR}/utiluti (local directory)"
        echo "  - /usr/local/bin/utiluti (system location)"
        echo ""
        echo "Please install utiluti from:"
        echo "  https://github.com/scriptingosx/utiluti"
        echo ""
        echo "Or place the utiluti binary in the same directory as this script."
        echo "========================================="
        exit 1
    else
        logMe "Found utiluti at: ${UTI_COMMAND}"
    fi
}
